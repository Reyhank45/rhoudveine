...
// Entry point
void kernel_init(void (*print_fn)(const char*)) {
    g_print_fn = print_fn;
    ...
    for (;;) {
        ...
        if (my_strcmp(buf, "cdl") == 0 || my_strncmp(buf, "cdl ", 4) == 0) {
            ...
            const char *path = (buf[3] == ' ') ? buf + 4 : g_cwd;
        ...
        if (is_cd && target_arg) {
            ...
            if (target_arg[0] == '/') {
                int i = 0;
                while(target_arg[i] && i < 127) {
                    new_path[i] = target_arg[i];
                    i++;
                }
                new_path[i] = '\0';
            } else {
                // Relative: concat g_cwd + / + target_arg
                int pos = 0;
                // Copy cwd
                while(g_cwd[pos]) { new_path[pos] = g_cwd[pos]; pos++; }
                
                // Add separator if needed
                if (pos > 0 && new_path[pos-1] != '/' && pos < 127) {
                    new_path[pos++] = '/';
                }
                
                // Copy target
                int k = 0;
                while(target_arg[k] && pos < 127) {
                    new_path[pos++] = target_arg[k++];
                }
                new_path[pos] = '\0';
            }
            
            // Normalize: remove trailing slash if length > 1 (keep root "/")
            int len = my_strlen(new_path);
            if (len > 1 && new_path[len-1] == '/') {
                new_path[len-1] = '\0';
            }
            
            // Update g_cwd
            // Note: In a real shell we would vfs_open it to verify it exists and is a directory.
            // For now, we trust the user or the "ls" command will fail later.
            // But let's check strict length
            if (len < 128) {
                for(int i=0; i<=len; i++) g_cwd[i] = new_path[i];
                out_puts("Changed directory to: ");
                out_puts(g_cwd);
                out_putchar('\n');
            } else {
                out_puts("Path too long\n");
            }
            continue;
        }
